...
def update_portal():
    """results 폴더를 스캔하여 메인 포털(index.html)을 생성합니다."""
    output_dir = "./results"
    # 날짜별 리포트 파일 목록 수집 (morning_report_YYYYMMDD_HHMM.html)
...
<head>
    ...
    <style>
        ...
        .archive-section {{ display: grid; grid-template-columns: 1fr; gap: 30px; }}
        ...
        /* Simple List for now (Expandable to real JS Calendar) */
...
<body>
    ...
    <div class="container">
        <div class="hero-card">
            ...
            <a href="results/{latest_filename}" class="btn">지금 읽기</a>
        </div>

        <div class="calendar-card">
            <div class="calendar-title">리포트 아카이브</div>
            <ul class="report-list">
                {"".join([f'<li class="report-item"><span class="report-date">{d[:4]}-{d[4:6]}-{d[6:8]}</span><a href="results/{file_links[d]}" class="report-link">보기</a></li>' for d in sorted(file_links.keys(), reverse=True)])}
            </ul>
        </div>
    </div>
...
"""
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(portal_html)
    print("✅ 포털 페이지(index.html) 업데이트 완료!")
